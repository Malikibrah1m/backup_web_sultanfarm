...
class MobileTransaksiController extends Controller
{
    ...
    {
        $plasmas = DB::table('plasmas')
            ->select('plasmas.*')
            ->get();

        return response()->json($plasmas);
    }
    ...
    public function update_kamar_tidak_tersedia(Request $request)
    {
        ...
        // Hitung waktu kamar akan kembali tersedia (contoh: 1 jam dari sekarang)
        $waktuKembaliTersedia = Carbon::now()->addMinutes(5);

        // Jadwalkan pekerjaan untuk mengubah status kamar kembali ke "Tersedia" setelah satu jam
        SetKamarAvailableJob::dispatch($kamarId)->delay($waktuKembaliTersedia);
        ...
        if ($kamar) {
            if ($kamar->status == $status) {
                ...
                DB::table('kamars')->where('id', $kamarId)->update([
                    ...
                    'waktu_kembali_tersedia' => $waktuKembaliTersedia
                ]);
    ...
    public function transaksi(Request $request)
    {
        ...
        DB::table('kamars')->where(
            'id',
            $request->input('id_kamar')
        )->update([
        ...
        $plasma = Plasma::find($request->input('id_plasma'));
